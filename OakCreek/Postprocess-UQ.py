...
# model_labels = [r'MDN$_{\text{LSTM}}$'+f'-{i}' for i in range(10)] + \
#                [r'MDN$_{Q,\text{LSTM}}$'+f'-{i}' for i in range(10)]
model_labels = [r'MDN$_{\text{LSTM}}$'+f'-{i}' for i in range(9)] + \
               [r'MDN$_{Q}$'+f'-{i}' for i in range(9)] + \
               [r'MDN$_{Q,\text{LSTM}}$'+f'-{i}' for i in range(9)]
...
# %%
df_data = pd.read_csv('data_withDiffusion_withSpinup-correctedZeroFlow.csv', index_col=0)
...
cutoff_length = 365
df_data_cut = df_data.iloc[cutoff_length:]

# Other arguments for running the model
nt = df_data_cut.shape[0]
sTmT_init = jnp.zeros([nt, 2])
J, Q, ET, C_J, C_Q_J, dS, time = get_transport_obs_fluxes(df_data_cut, 'Oakcreek', return_dS=True)


# %%
# Load the trained SAS parameters
with open("models-withDiffusion-rev/storage-dependent-gamma-mse.json", "r") as f:
    sas_configs = json.load(f)
sas_param = sas_configs['x']

# SAS functions
sas_Q = SAS_Gamma_StorageDependent(a=sas_param[0], λ=sas_param[1], ΔScλ=sas_param[2])
sas_ET = SAS_Uniform(scale=sas_param[3])
...
# Run the model
transport_output = sas_transport(J, C_J, Q, ET, sTmT_init, sas_Q_args, sas_ET_args)
...
df_data_cut['C_Q_sim'] = transport_output[-1]
df_data_cut['C_Q_simb'] = transport_output_op[-1]
df_set.append(df_data_cut)
...
for i,model_type in enumerate(model_type_set):
    ...
    ax1, ax2 = plot_timeseries_obs_1to1(
        obs=df[test_s:test_e]['C_Q'].values, sim=df[test_s:test_e]['C_Q_sim'].values, lim=[-0.05,1], 
        timesteps=df[test_s:test_e].index, units='Tracer \n [mol mol$^{-1}$]', varn=model_type, 
        label_sim='DiffSAS', label_obs='ATS', axes=[ax1, ax2], legend=False, figsize=(12, 3)
...
for i,model_type in enumerate(model_type_set):
    ...
    ax1, ax2 = plot_timeseries_obs_1to1(
        obs=df[test_s:test_e]['C_Q'].values, sim=df[test_s:test_e]['C_Q_simb'].values, lim=[-0.05,1], 
        timesteps=df[test_s:test_e].index, units='Tracer \n [mol mol$^{-1}$]', varn=model_type + " (operational mode)", 
        label_sim='DiffSAS', label_obs='ATS', axes=[ax1, ax2], legend=False, figsize=(12, 3)
        # label_sim='DiffSAS', figsize=(10,2)
    );
    if i != len(model_type_set)-1:
